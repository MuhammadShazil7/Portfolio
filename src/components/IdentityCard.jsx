...
// new id card with improvements

// components/IdentityCard.jsx
import Tilt from "react-parallax-tilt";
import { motion } from "framer-motion";
import { FiGithub, FiLinkedin, FiMail, FiMapPin, FiCode, FiClock, FiHeart } from "react-icons/fi";
import { FaReact, FaNodeJs, FaJava } from "react-icons/fa";
import { SiMongodb, SiTypescript, SiTailwindcss, SiDotnet, SiPhp, SiLaravel } from "react-icons/si";
...
export default function IdentityCard() {
  ...
  // My actual tech stack (not just AI-generated ones)
  const techStack = [
    { name: "React", icon: FaReact, color: "#61DAFB" },
    { name: ".NET", icon: SiDotnet, color: "#512BD4" },
    { name: "Node.js", icon: FaNodeJs, color: "#339933" },
  ...
  return (
    <motion.div animate={floatAnimation} className="relative">
      ...
      >
        ...
        ">
          {/* Glow Orbs */}
          <div className="absolute -top-20 -right-20 w-60 h-60 bg-purple-500/20 rounded-full blur-[100px]" />
          ...
          <div className="relative z-10 px-6 pb-6 pt-2">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-slate-400 text-sm">I build things. Sometimes they work.</p>
              </div>
            </div>
            ...
            <div className="mt-3 space-y-1.5">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-500">
                ...
                <span>2+ years of debugging</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-slate-500">
                <FiHeart className="text-purple-400" />
                <span>100% self-taught (with YouTube)</span>
              </div>
            </div>

            {/* Tech Stack */}
            <div className="mt-4">
              <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-2">Stuff I use</p>
              <div className="grid grid-cols-3 gap-1.5">
                {techStack.map((tech, index) => (
                  <motion.div
                    key={tech.name}
                    ...
                    className="bg-white/5 rounded-xl p-2 border border-white/5 flex items-center justify-center gap-1.5 transition-all duration-300 group"
                  >
                    <tech.icon className="text-xs transition-transform duration-300 group-hover:scale-110" style={{ color: tech.color }} />
                    <span className="text-[9px] text-slate-400 font-medium">{tech.name}</span>
                  </motion.div>
                ))}
              </div>
            </div>
